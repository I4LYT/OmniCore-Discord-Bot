...
use poise::{
    serenity_prelude::{
        Colour, CreateEmbed, GuildId, Timestamp
    },
    CreateReply,
};
...
pub(crate) async fn set_prefix(
    ...
    let per_guild_settings_col = get_collection("per_guild_settings").expect("Failed to load per_guild_settings collection");

    if per_guild_settings_col.find_one(doc! {"guild_id": ctx.guild_id().unwrap().to_string()}).await.unwrap().is_none() {
        setup_guild(ctx.guild_id().unwrap()).await;
    }

    let _ = per_guild_settings_col.update_one(
...
pub(crate) async fn get_prefix(id: GuildId) -> String {
    ...
    let prefix = per_guild_settings_col.find_one(doc! {"guild_id": id.to_string()}).await.unwrap();
    ...
    let prefix = prefix.unwrap().get_str("prefix").unwrap().to_string().replace("\"", "");
