...
use poise::{Command, CreateReply, FrameworkError, serenity_prelude::{self as serenity, Colour, CreateEmbed, GuildId, Timestamp}};
...
impl EventHandler for Handler {
    async fn ready(&self, _: Context, ready: Ready) {
        log::info!("Shard {} is connected to {}", ready.shard.unwrap().id, ready.user.name);
    }
}
...
async fn get_guild_owner_id(ctx: &CustomContext<'_>) -> serenity::UserId {
    if ctx.guild_id().is_none() {
        return serenity::UserId::new(1); // Invalid so ntohing can point to it.
    }
...
async fn main() {
    ...
    config::init_config();
    let _ = database::mongo_connect().await.expect("Failed to connect to MongoDB");
    let _ = database::ensure_indexes().await.expect("Failed to ensure indexes");
        ...
        .options(poise::FrameworkOptions {
            ...
            command_check: Some(|ctx| {
                Box::pin(async move {
                    ...
                    if ctx.command().qualified_name == "set_prefix" && !(ctx.author().id == get_guild_owner_id(&ctx).await) {
            ...
            on_error: |err| {
                Box::pin(async move {
                    ...
                    match err {
                        FrameworkError::CommandCheckFailed {error: _, ctx: _, ..} => {skip = true},
                        _ => {}
                    }
                ...
                dynamic_prefix: Some(|ctx| {
                                    Box::pin(async move {
                                        if ctx.guild_id.is_none() { // is_none() is true if the command was executed in a DM
                                            ...
                                            let prefix = crate::set_prefix::get_prefix(guild).await;
...
async fn setup_guild(guild: GuildId) {
    ...
    let per_guild_settings_col = database::get_collection("per_guild_settings").expect("Failed to load per_guild_settings collection");
    ...
    if per_guild_settings_col.find_one(doc! {"guild_id": guild_id.to_string()}).await.unwrap().is_none() {
