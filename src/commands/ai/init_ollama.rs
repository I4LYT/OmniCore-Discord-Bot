...
use crate::config::{OLLAMA_MODEL, OLLAMA_BASE_URL};
use crate::OLLAMA;
use reqwest::Client;
...
pub(crate) async fn init_ollama() {
    ...
    if let Err(e) = response {
        panic!("Failed to ping Ollama server (check if Ollama is running and public): {}", e);
    }

    // Create the Ollama client
    let ollama = Ollama::try_new(ollama_base_url).expect("Failed to initialize Ollama client, check your OLLAMA_BASE_URL");
    ...
    let models = ollama.list_local_models().await.expect("Failed to list Ollama models").iter().map(|m| m.name.clone()).collect::<Vec<String>>();
    
    if !models.contains(&ollama_model) {
        log::error!("Ollama model {} does not exist, check your OLLAMA_MODEL", ollama_model);
