...
pub(crate) async fn on_mention(
    ...
    let Some(guild_id) = msg.guild_id else {
        msg.reply(ctx.http.clone(), "I only work inside servers, not DMs.").await?;
        return Ok(());
    };
    ...
    let res = match res {
        ...
        Err(e) => {
            ...
            msg.reply(ctx.http.clone(), "Sorry, I couldn't process that — something went wrong on my end.").await?;
            return Err(e.into());
        }
    };
    ...
    let sent = match msg.reply(ctx.http.clone(), res.message.content.clone()).await {
        Ok(sent) => sent,
        Err(e) => {
            log::warn!(
